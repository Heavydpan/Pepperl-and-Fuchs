...
void R2000DET::R2000StatusCallback(const sensor_msgs::LaserScan::ConstPtr& msg)
{
	...
	ros::NodeHandle n;//节点Handle
	ros::Publisher r2000detdata_pub = n.advertise<std_msgs::Int8MultiArray>("R2000Status", 1000);
...
bool* R2000DET::isInZone_ROS(const sensor_msgs::LaserScan::ConstPtr& PointBuffer)
{
	...
	for (uint32_t i1 = 0; i1 <PointBuffer->ranges.size(); i1++)
	{
		...
		distance = PointBuffer->ranges[i1];
...
bool R2000DET::isInOneZone(float tmpx, float tmpy, double x, double y)
{
	bool flag = false;
	uint64_t *ds = nullptr;
